...
function initWorkflowBuilder(root) {
    ...
    const deleteModalText = document.querySelector('[data-delete-text]');
    ...
    function bindCanvas() {
        if (!viewport || !canvas || !clearCanvasButton) {
            return;
        }
        ...
        viewport.addEventListener('mousedown', (event) => {
            ...
            document.body.classList.add('builder-is-panning');
        });
        ...
        window.addEventListener('mouseup', () => {
            ...
            document.body.classList.remove('builder-is-dragging-node');

            if (state.panActive) {
                state.panActive = false;
                viewport.classList.remove('is-panning');
                document.body.classList.remove('builder-is-panning');
            }
        });
        ...
        viewport.addEventListener('wheel', (event) => {
            ...
            event.preventDefault();
            adjustZoom(event.deltaY < 0 ? 1.04 : 1 / 1.04, event.clientX, event.clientY);
        }, { passive: false });
    ...
    function bindZoom() {
        // Zoom e controlado pelo wheel com Ctrl/Cmd dentro do viewport.
    }

    function bindDeleteModal() {
        if (!deleteModal || !deleteModalText || !deleteConfirmButton || !deleteCancelButton) {
            return;
        }
    ...
    function bindEditor() {
        if (!selectedPanel || !removeStepButton || !saveForm) {
            return;
        }
        ...
        fields.forEach((field) => {
            field.addEventListener('input', () => {
                ...
                if (!node) return;
                node[field.dataset.stepField] = field.value;
                render();
            });
        });
        ...
        saveForm.addEventListener('submit', (event) => {
            try {
                ...
                if (!workflowName.value.trim() || !workflowSlug.value.trim()) {
                    throw new Error('Nome e slug do workflow sao obrigatorios.');
                }
            } catch (error) {
                event.preventDefault();
    ...
    function loadWorkflow(workflow) {
        ...
        const editorViewport = workflow.definition?.editor?.viewport ?? null;
        ...
        if (editorViewport) {
            state.transform.scale = editorViewport.scale ?? 1;
            state.transform.x = editorViewport.x ?? 0;
            state.transform.y = editorViewport.y ?? 0;
        }
    ...
    function renderNodes() {
        ...
        state.nodes.forEach((node) => {
            ...
            element.style.left = `${node.x + WORLD_PADDING}px`;
            element.style.top = `${node.y + WORLD_PADDING}px`;
            ...
            element.querySelectorAll('[data-port="out"]').forEach((port) => {
                port.addEventListener('click', (event) => {
                    ...
                    if (state.connectSource?.nodeId === node.id && state.connectSource?.side === side) {
                        ...
                        state.connectSource = { nodeId: node.id, side };
                    }
            ...
            element.querySelectorAll('[data-port="in"]').forEach((port) => {
                port.addEventListener('click', (event) => {
                    event.stopPropagation();
                    if (!state.connectSource || state.connectSource.nodeId === node.id) return;
            ...
            handle.addEventListener('mousedown', (event) => {
                ...
                document.body.classList.add('builder-is-dragging-node');
            });
    ...
    function updateConnectHint() {
        ...
        if (!state.connectSource) {
            connectHint.textContent = 'Nenhuma conexao em andamento.';
            return;
        }

        const node = state.nodes.find((item) => item.id === state.connectSource.nodeId);
        connectHint.textContent = `Conectando saida ${state.connectSource.side} de ${node?.title ?? 'agente'}. Escolha a entrada do proximo no.`;
    }

    function connectNodes(fromId, toId, fromSide, toSide) {
        if (state.edges.some((edge) => edge.to === toId)) {
            showToast('Cada passo pode receber apenas uma conexao de entrada.', 'error');
            return;
        }

        if (state.edges.some((edge) => edge.from === fromId)) {
            showToast('Cada passo pode ter apenas uma saida no builder linear.', 'error');
            return;
        }

        if (createsCycle(fromId, toId)) {
            showToast('Essa ligacao criaria um ciclo no workflow.', 'error');
            return;
        }
    ...
    function orderNodes() {
        ...
        if (state.edges.length !== Math.max(0, state.nodes.length - 1)) {
            throw new Error('O builder atual exige um fluxo linear: conecte todos os passos em uma unica sequencia.');
        }
        ...
        while (current) {
            if (visited.has(current.id)) {
                throw new Error('O workflow possui ciclo. Remova as conexoes invalidas.');
            }
            ...
            if (nextIds.length > 1) {
                throw new Error('O builder atual nao suporta branching. Deixe apenas uma saida por passo.');
            }
        ...
        if (ordered.length !== state.nodes.length) {
            throw new Error('Ha passos soltos no canvas. Conecte todos antes de salvar.');
        }
    ...
    function openDeleteModal(nodeId, nodeTitle) {
        ...
        deleteModalText.textContent = `Remover "${nodeTitle}" do canvas? As conexoes ligadas a ele tambem serao removidas.`;
    ...
    function screenToCanvas(clientX, clientY) {
        ...
        return {
            x: ((clientX - rect.left + WORLD_PADDING - state.transform.x) / state.transform.scale) - WORLD_PADDING,
            y: ((clientY - rect.top + WORLD_PADDING - state.transform.y) / state.transform.scale) - WORLD_PADDING,
        };
    }

    function adjustZoom(factor, clientX, clientY) {
        const nextScale = Number((state.transform.scale * factor).toFixed(4));

        if (!Number.isFinite(nextScale) || nextScale <= 0) {
            return;
        }

        if (viewport && Number.isFinite(clientX) && Number.isFinite(clientY)) {
            const rect = viewport.getBoundingClientRect();
            ...
            state.transform.x = pointerX + WORLD_PADDING - ((world.x + WORLD_PADDING) * state.transform.scale);
            state.transform.y = pointerY + WORLD_PADDING - ((world.y + WORLD_PADDING) * state.transform.scale);
            applyTransform();
            return;
        }
    ...
    function updateCanvasBounds() {
        ...
        const width = viewport.clientWidth + (WORLD_PADDING * 2);
        const height = viewport.clientHeight + (WORLD_PADDING * 2);

        canvas.style.width = `${width}px`;
        canvas.style.height = `${height}px`;
        canvas.style.left = `-${WORLD_PADDING}px`;
        canvas.style.top = `-${WORLD_PADDING}px`;
    }
}
...
function getAnchorPoint(node, side, width, height) {
    ...
    switch (side) {
        case 'top':
            return { x: node.x + WORLD_PADDING + width / 2, y: node.y + WORLD_PADDING };
        case 'bottom':
            return { x: node.x + WORLD_PADDING + width / 2, y: node.y + WORLD_PADDING + height };
        case 'left':
            return { x: node.x + WORLD_PADDING, y: node.y + WORLD_PADDING + height / 2 };
        case 'right':
        default:
            return { x: node.x + WORLD_PADDING + width, y: node.y + WORLD_PADDING + height / 2 };
    }
}
